...
class OfferStatusModal extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return userOfferAsync.when(
      ...
      error: (err, stack) => SizedBox(
          height: 300,
          child: Center(child: Text("${ItemDetailsStrings.errorPrefixGeneric}$err"))
      ),
      data: (liveOffer) {
        // ✅ Ensure we use live data if available, fallback to constructor
        final currentOffer = liveOffer ?? myOffer;
        final status = currentOffer['status'] ?? ItemDetailsStrings.statusPending;
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      AppValues.gapXL,
                      Text(
                          ItemDetailsStrings.whatHappensNext,
  ...
  Widget _buildTimeline(BuildContext context, String status) {
    ...
    Color step1Color = AppColors.royalBlue.withAlpha(25);
    ...
    if (status == ItemDetailsStrings.statusAcceptedValue) {
      // Step 2: Blue Check
      step2Color = AppColors.royalBlue.withAlpha(25);
      step2Icon = AppColors.royalBlue;
      step2IconData = Icons.check_circle;

      // Step 3: Blue Chat
      step3Color = AppColors.royalBlue.withAlpha(25);
      step3Icon = AppColors.royalBlue;
    } else if (status == ItemDetailsStrings.statusRejectedValue) {
      // Step 2: Red X
      step2Color = AppColors.errorColor.withAlpha(25);
    ...
    return Column(
      children: [
        ...
        _buildTimelineStep(
          ...
          color: step2Color,
          iconColor: step2Icon,
          title: status == ItemDetailsStrings.statusRejectedValue ? ItemDetailsStrings.offerRejectedTitle : ItemDetailsStrings.sellerReview, // Change text if rejected
          subtitle: status == ItemDetailsStrings.statusRejectedValue
              ? ItemDetailsStrings.offerDeclinedSubtitle
  ...
  Widget _buildHeader(BuildContext context, String status) {
    ...
    if (status == ItemDetailsStrings.statusAcceptedValue) {
      ...
      title = ItemDetailsStrings.offerRejectedTitle; // Updated Title
      subtitle = ItemDetailsStrings.offerNotAcceptedSubtitle;
    ...
    return Column(
      ...
      children: [
        Text(
          title,
          style: AppTextStyles.titleSmall(context, bold: true, color: color),
        ),
        AppValues.gapXS,
        Text(
          subtitle,
  ...
  Widget _buildComparisonCard(BuildContext context, Map<String, dynamic> offerData) {
    ...
    final sellerName = sellerProfile['username'] ?? ItemDetailsStrings.defaultSellerName;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: AppColors.greyLight.withAlpha(128),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(ItemDetailsStrings.labelYourItem, style: AppTextStyles.labelSmall(context, color: AppColors.textGrey)),
                Text(
                  swapImage.isEmpty && hasCash ? ItemDetailsStrings.labelCashOffer : swapTitle,
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                  style: AppTextStyles.bodyMedium(context, bold: true),
                ),
                AppValues.gapS,
                _buildItemImage(
                    context,
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                 Text(ItemDetailsStrings.labelSellerItem,
                ...
                AppValues.gapS,
                _buildItemImage(context, imageUrl: sellerImage),
                AppValues.gapS,
                Text(
                  "${ItemDetailsStrings.sellerPrefix}$sellerName",
                  style: AppTextStyles.labelSmall(context, color: AppColors.textGrey),
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                ),
              ],
  ...
  Widget _buildItemImage(BuildContext context, {required String imageUrl, String? cashBadge, bool isCashOnly = false}) {
    return Column(
      children: [
        Container(
          height: 100,
        ...
        if (cashBadge != null) ...[
          Transform.translate(
            ...
            child: Container(
              ...
              child: Text(
                cashBadge,
                style: AppTextStyles.bodySmall(context, bold: true),
              ),
            ),
          )
        ]
      ...
      child: Row(
        children: [
          ...
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(title, style: AppTextStyles.bodyMedium(context, bold: true)),
              Text(subtitle, style: AppTextStyles.bodySmall(context, color: AppColors.textGrey)),
            ],
          )
        ],
      ),
  ...
  Widget _buildFooterButton(BuildContext context, String status, Map<String, dynamic> offer) {
    ...
    if (status == ItemDetailsStrings.statusAcceptedValue) {
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (_) => DealChatScreen(
              ...
              itemName: listingItem['title'] ?? ItemDetailsStrings.defaultItemNameAlt,
      ...
      text = ItemDetailsStrings.btnRejectedTitle;
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        child: Text(text, style: AppTextStyles.bodyLarge(context, bold: true, color: textColor)),
      ),
    );
  }
}
